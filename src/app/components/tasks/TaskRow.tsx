...
const TaskRow = ({task}:Props) => {
  ...
  return (
    <tr className="hover:bg-muted/30">
        ...
        {/* Status */}
        <td className="px-4 py-2">
        <Select value={editingTask.status} onValueChange={(value) => handleChange("status", value)}>
          ...
          {/* Priority */}
      <td className="px-4 py-2">
        <Select value={editingTask.priority} onValueChange={(value) => handleChange("priority", value)}>
      ...
      <td className="px-4 py-2 text-right">
        <Button variant="ghost" size="icon" onClick={handleDelete}>
          {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2Icon className="w-4 h-4 text-destructive" />}
        </Button>
      </td>
      </td>
    </tr>
